...
class ModsManager(private val adbClient: AdbClient) {
    ...
    suspend fun installModToQuest(
        ...
        try {
            ...
            val filesToCopy = if (extractedModDir.isDirectory) {
                extractedModDir.listFiles()?.toList() ?: emptyList()
            } else {
                listOf(extractedModDir)
            }
            
            if (filesToCopy.isEmpty()) {
                return ModInstallResult(
                    success = false,
                    message = "لا توجد ملفات لنقلها"
                )
            }
            ...
            for (file in filesToCopy) {
                onProgress("نسخ ${file.name} (${copiedFiles + 1}/$totalFiles)...")
                
                val targetFile = "$targetPath/${file.name}"
                val pushResult = adbClient.push(serial, file, targetFile)
                
                if (pushResult.exit != 0) {
                    return ModInstallResult(
                        success = false,
                        message = "فشل نسخ ${file.name}: ${pushResult.err}"
                    )
                }
            ...
            return ModInstallResult(
                success = true,
                message = "تم تثبيت المود بنجاح! تم نسخ $copiedFiles ملفات"
            )
